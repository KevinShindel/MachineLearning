...
from adtk.detector import OutlierDetector, ThresholdAD
...
def main():
    ...
    test = df[
        "2020":"2020"
    ]  # тестовые данные который используются для проверки обучения модели
    print(test.head(5))

    # сезонность 12 периодов методотом Хольта Винтерса
    fit1 = ExponentialSmoothing(
        endog=train_df, seasonal_periods=12, trend="add", seasonal="mul"
    ).fit()
    print(fit1.params)
    ...
    ax = df.plot(
        figsize=(15, 6),
        color="black",
        title="Прогноз методотом Хольта Винтерса",
        label="фактические значения",
    )

    # показать смоделированые значения
    fit1.fittedvalues.plot(
        ax=ax, style="--", color="red", label="Смоделированые значения"
    )

    # предсказать 12 периодов ( 1 год )
    fit1.forecast(12).plot(
        ax=ax, style="--", color="green", label="Предсказаные значения"
    )
    ax.legend(loc="best")
    plt.show()  # чёрные - фактические значения, красные - смоделированые значения, зелёные - предсказаные.

    fit2 = ExponentialSmoothing(
        endog=train_df, seasonal_periods=12, trend="mul", seasonal="mul"
    ).fit()  # сезонность 12 периодов методотом Хольта Винтерса
    fit3 = ExponentialSmoothing(
        endog=train_df, seasonal_periods=12, trend="mul", seasonal="add"
    ).fit()  # сезонность 12 периодов методотом Хольта Винтерса
    fit4 = ExponentialSmoothing(
        endog=train_df, seasonal_periods=12, trend="add", seasonal="add"
    ).fit()  # сезонность 12 периодов методотом Хольта Винтерса
    ...
    fit2.fittedvalues.plot(
        ax=ax, style="--", color="blue", label="Смоделированые значения"
    )

    # предсказать 12 периодов ( 1 год )
    fit2.forecast(12).plot(
        ax=ax, style="--", color="purple", label="Предсказаные значения"
    )
    ax.legend(loc="best")

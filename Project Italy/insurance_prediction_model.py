...
def forecast_warming():
    ...
    df["avg_anomaly_temp"] = (
        df["avg_anomaly_temp"] + abs(df["avg_anomaly_temp"].min()) + 1
    )

    # Train the model on data up to 2024
    train = df.loc[:"2020", "avg_anomaly_temp"]
    model = ExponentialSmoothing(
        train, seasonal_periods=3, trend="mul", seasonal="add"
    ).fit()
...
def forecast_claims(e_df=None, w_df=None):
    ...
    corr1 = all_in_df["emissions"].corr(
        all_in_df["gross"]
    )  # negative correlation: -0.477 - means that as emissions grow, gross index decreases
    corr2 = all_in_df["avg_anomaly_temp"].corr(
        all_in_df["gross"]
    )  # positive correlation 0.26 - means that as temperature grows, gross index grows
    ...
    plt.title(
        "Correlation of Gross Claims Expenditures to Emissions in Italy 2004 – 2020",
        x=0.6,
    )
    plt.tight_layout()
    ...
    model = ExponentialSmoothing(
        x_train, seasonal_periods=3, trend="mul", seasonal="add"
    ).fit()
    ...
    plt.legend()
    plt.title(
        "Gross Claims Expenditures Predicted vs Real in Italy 2004 – 2020 (based on historical data)",
        fontsize=9,
        x=0.4,
    )
    plt.tight_layout()
    ...
    plt.ylabel("Gross Claims Expenditures")
    plt.title(
        "Gross Claims Expenditures Predicted 2020 – 2028 (based on the historical data 2004 – 2020)",
        fontsize=9,
        x=0.4,
    )
    plt.legend()
...
if __name__ == "__main__":
    ...
    print(
        "Correlation between CO2 emissions and global warming: ", corr
    )  # considered high correlation : 0.705
    ...
    plt.title(
        "Correlation of Global Warming to Emissions in Italy 1860 – 2020",
        fontsize=11,
        x=0.6,
    )
    plt.tight_layout()

...
class TaxiDurationPredictor(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input):
        ...
        if self.model is None:
            raise ValueError(
                "Model not loaded. Please ensure load_context() is called."
            )
...
def log_model_to_mlflow(
    model_path: str, tracking_uri: str = "http://127.0.0.1:5000"
) -> str:
    ...
    try:
        ...
        conda_env = {
            ...
            "dependencies": ["python=3.11", "scikit-learn", "pandas", "joblib"],
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Model is being served at http://localhost:{SERVE_PORT}/invocations"
        )
        ...
        logger.info(
            """curl -X POST -H "Content-Type: application/json" -d "{\"inputs\": [20.0]}"
         http://127.0.0.1:"""
            + str(SERVE_PORT)
            + """/invocations"""
        )

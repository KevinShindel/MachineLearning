...
def promote_model():
    ...
    client.set_model_version_tag(
        name=MODEL_NAME, version=version, key="status", value="staging"
    )

    # Example 2: Add metadata tags
    client.set_model_version_tag(
        name=MODEL_NAME, version=version, key="promoted_by", value="data_science_team"
    )
    ...
    for mv in versions:
        ...
        if (
            hasattr(model_version, "tags")
            and model_version.tags.get("status") == "production"
        ):
            client.set_model_version_tag(
                name=MODEL_NAME, version=mv.version, key="status", value="archived"
            )

    # Set new version as production
    client.set_model_version_tag(
        name=MODEL_NAME, version=version, key="status", value="production"
    )

...
def main():
    ...
    plot(df, anomaly=th_anomalies, ts_linewidth=1, ts_markersize=3, anomaly_markersize=5, anomaly_color='red', anomaly_tag="marker")
    plt.show()
    ...
    plot(df, anomaly=od_anomalies, ts_linewidth=2, ts_markersize=3, anomaly_markersize=5, anomaly_color='red', anomaly_tag='marker', ts_alpha=0.3, curve_group='all')
    plt.show()
    ...
    fit1 = ExponentialSmoothing(endog=train_df, seasonal_periods=12, trend='add', seasonal='mul').fit()  # сезонность 12 периодов методотом Хольта Винтерса
    print(fit1.params)
    ...
    ax = df.plot(figsize=(15, 6), color='black', title='Прогноз методотом Хольта Винтерса', label='фактические значения')  # постройка графика на основании осонвного фрейма
    fit1.fittedvalues.plot(ax=ax, style='--', color='red', label='Смоделированые значения')  # показать смоделированые значения
    fit1.forecast(12).plot(ax=ax, style='--', color='green', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    ax.legend(loc='best')
    ...
    fit1.fittedvalues.plot(ax=ax, style='--', color='red', label='Смоделированые значения')  # показать смоделированые значения
    fit1.forecast(12).plot(ax=ax, style='--', color='green', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    fit2.fittedvalues.plot(ax=ax, style='--', color='blue', label='Смоделированые значения')  # показать смоделированые значения
    fit2.forecast(12).plot(ax=ax, style='--', color='purple', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    ax.legend(loc='best')
    plt.show()  # чёрные - фактические значения, красные - смоделированые значения, зелёные - предсказаные.
    ...
    fit1.fittedvalues.plot(ax=ax, style='--', color='red', label='Смоделированые значения')  # показать смоделированые значения
    fit1.forecast(12).plot(ax=ax, style='--', color='green', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    fit3.fittedvalues.plot(ax=ax, style='--', color='blue', label='Смоделированые значения')  # показать смоделированые значения
    fit3.forecast(12).plot(ax=ax, style='--', color='purple', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    ax.legend(loc='best')
    plt.show()  # чёрные - фактические значения, красные - смоделированые значения, зелёные - предсказаные.
    ...
    fit1.fittedvalues.plot(ax=ax, style='--', color='red', label='Смоделированые значения')  # показать смоделированые значения
    fit1.forecast(12).plot(ax=ax, style='--', color='green', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    fit4.fittedvalues.plot(ax=ax, style='--', color='blue', label='Смоделированые значения')  # показать смоделированые значения
    fit4.forecast(12).plot(ax=ax, style='--', color='purple', label='Предсказаные значения')  # предсказать 12 периодов ( 1 год )
    ax.legend(loc='best')
    plt.show()  # чёрные - фактические значения, красные - смоделированые значения, зелёные - предсказаные.
